...
// Copyright (C) 2015-2021 Johan Bijnens, v1.0
...
double jbdsing15(double (*f)(double,void*),const double a,const double b,
  ...
  if( (s==a) || (s==b)){
    ...
    if( (s< fmin(a,b)) || (s > fmax(a,b))){
      h= jbdquad15(f,a,b,eps,ap);
      return h;}
    else{
      if(2.*s<(a+b)) {
        h= jbdquad15(f,2.*s-a,b,eps,ap);
        b0=s-a;}
      else{
        h= jbdquad15(f,a,2.*s-b,eps,ap);
        b0=b-s;}
      c=cst/b0;
      bb=0.;
      ndiv = 1;      
  ...
  if(ndiv<=2) {
    ...
    label2:
      ...
      for (int i=0; i<=2;i++){
	u=c2*xgk[2*i+1];
	s0=((*f)(c3+u,ap)+(*f)(c4-u,ap))+((*f)(c3-u,ap)+(*f)(c4+u,ap));
	s8=s8+wg[i]*s0;
	s16=s16+wgk[2*i+1]*s0;
      }
      s8=c2*s8;
      for (int i=0; i<=3;i++){
	u=c2*xgk[2*i];
	s16=s16+wgk[2*i]*(((*f)(c3+u,ap)+(*f)(c4-u,ap))+((*f)(c3-u,ap)+(*f)(c4+u,ap)));
      }
      s16=c2*s16;
      if(fabs(s16-s8)<(eps*(1.+fabs(s16)))) goto label5;
      ndiv += 1;
