...
// automatically as needed and now a void* added to move around
// extra variables, complex variables instead of real here
...
void jbdadmul(dcomplex (*f)(dcomplex x[],void* ),void* ap,const int n,
	      dcomplex a[],dcomplex b[],
	      const int minpts,const int maxpts,const double eps,dcomplex wk[],
	      const int iwk,dcomplex &result,double &relerr,
	      int &nfnevl,int &ifail);

dcomplex jbdad2(dcomplex (*fcn)(dcomplex x[],void*),void* ap,dcomplex a[],dcomplex b[],
  ...
  dcomplex wk[iwk+1];
  dcomplex result;
  jbdadmul(fcn,ap,ndim,a,b,minpts,maxpts,releps,wk,iwk,result,relerr,nfnevl,ifail);
  if(ifail!=0){
...
// due to the very large array, only use 2000000 points
dcomplex jbdad3(dcomplex (*fcn)(dcomplex x[],void*),void* ap, dcomplex a[],dcomplex b[],
	      const double releps, double &relerr,int &ifail){
  const int ndim = 3;
...
// due to the very large array, only use if really necessary
// use ulimit -s stacksize in kB before running it to get a larger stack
// if needed
dcomplex jbdad4(dcomplex (*fcn)(dcomplex x[],void*),void* ap,dcomplex a[],dcomplex b[],
	      const double releps, double &relerr,int &ifail){
  const int ndim = 4;
...
void jbdadmul(dcomplex (*f)(dcomplex x[],void* ),void* ap, const int n,
	      dcomplex aa[],dcomplex bb[],
	      const int minpts,const int maxpts,const double eps,dcomplex wk[],
	      const int iwk,dcomplex &result,double &relerr,
	      int &nfnevl,int &ifail){
 
  bool ldv;
  dcomplex ctr[16],wth[16],wthl[16],z[16],a[16],b[16];
  ...
  double rgnerr,difmax,dif;
  dcomplex rgnvol,rgncmp,rgnval,sum1,sum2,sum3,sum4,sum5,f2,f3;
...
label90:
  ...
  for(j=1;j<=n;j++){
    wthl[j]=-wthl[j];
    z[j]=ctr[j]+wthl[j];
    if(real(wthl[j]/(b[j]-a[j])) > 0.) goto label90;
  }
...
label110:
    ...
    if(isbtmp < isbrgs) {
      ...
      if(real(wk[isbtmp]) < real(wk[isbtpp])) isbtmp=isbtpp;
    }
    if(rgnerr >= real(wk[isbtmp])) goto label160;
...
label140:
  ...
  if((isbtmp >= irgnst) && (rgnerr > real(wk[isbtmp]))) {
    for(k=0;k <= irgnst-1;k++) wk[isbrgn-k]=wk[isbtmp-k];
    isbrgn=isbtmp;
    goto label140;
  }
label160:
  ...
  if(ifail == 3) {
    ldv=true;
    isbrgn=irgnst;
    abserr=abserr-real(wk[isbrgn]);
    result=result-wk[isbrgn-1];
    idvax0=real(wk[isbrgn-2]);
    for(j=1;j<=n;j++){
      isbtmp=isbrgn-2*j-2;
      ctr[j]=wk[isbtmp+1];
      wth[j]=wk[isbtmp];
