...
/* J.~Bijnens, first programmed 25/4/2002 
...
double jbdquad15(double (*f)(const double),const double a,const double b,
 ...
 label2:
  ...
  std::cout << "jbdquad15 too high accuracy required\n";
  return result;
label5:
  result = result+s2;
  if(bb != b) goto label1;
...
// version with an extra void pointer
double jbdquad15(double (*f)(const double,void*),const double a,const double b,
 ...
 label2:
  ...
  for (i=0;i<=2;i++){
    x=width*xgk[2*i+1];
    s0 = ((*f)(center+x,ap)+(*f)(center-x,ap));
    s1 = s1+ wg[i]*s0;
    s2 = s2+wgk[2*i+1]*s0;
  }
  for (i=0;i<=3;i++){
