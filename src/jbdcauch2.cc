...
double jbdcauch2(double (*f)(double),const double a,const double b,
  ...
  if( (s==a) || (s==b)){
      h=0.;
      std::cout << "singularity at border in jbdcauch2" << std::endl;
      return h;}
  else {
    if( (s< fmin(a,b)) || (s > fmax(a,b))){
      h= jbdgauss2(f,a,b,eps);
      return h;}
    else{
      if(2*s<(a+b)) {
        h= jbdgauss2(f,2.*s-a,b,eps);
        b0=s-a;}
      else{
        h= jbdgauss2(f,a,2.*s-b,eps);
        b0=b-s;}
      c=cst/b0;
      bb=0;
  ...
  if(ndiv<=2) {
	...
	s16=s16+w[i]*((f(c3+u)+f(c4-u))+(f(c3-u)+f(c4+u)));
      ...
      h=0.;
      std::cout << "jbdcauch2 too high accuracy required" << std::endl;
      goto label9;
    label5:
      h=h+s16;
...
//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
// version with void pointer

double jbdcauch2(double (*f)(double,void*),const double a,const double b,
  ...
  if( (s==a) || (s==b)){
    ...
    if( (s< fmin(a,b)) || (s > fmax(a,b))){
      h= jbdgauss2(f,a,b,eps,ap);
      return h;}
    else{
      if(2*s<(a+b)) {
        h= jbdgauss2(f,2.*s-a,b,eps,ap);
        b0=s-a;}
      else{
        h= jbdgauss2(f,a,2.*s-b,eps,ap);
        b0=b-s;}
      c=cst/b0;
      bb=0;
  ...
  if(ndiv<=2) {
    ...
    label2:
      ...
      for (int i=0; i<=4;i++){
	u=c2*x[i];
	s8=s8+w[i]*((f(c3+u,ap)+f(c4-u,ap))+(f(c3-u,ap)+f(c4+u,ap)));
      }
      s8=c2*s8;
      s16=0.;
      for (int i=5; i<=12;i++){
	u=c2*x[i];
	s16=s16+w[i]*((f(c3+u,ap)+f(c4-u,ap))+(f(c3-u,ap)+f(c4+u,ap)));
      }
      s16=c2*s16;
      if(fabs(s16-s8)<(eps*(1.+fabs(s16)))) goto label5;
      ndiv += 1;
