...
// Copyright (C) 2014-2021 Johan Bijnens, v1.01
...
double jbdcauch(double (*f)(double,void*),const double a,const double b,
  ...
  if( (s==a) || (s==b)){
    ...
    if( (s< fmin(a,b)) || (s > fmax(a,b))){
      h= jbdgauss(f,a,b,eps,ap);
    return h;}
    else{
      if(2*s<(a+b)) {
        h= jbdgauss(f,2.*s-a,b,eps,ap);
        b0=s-a;}
      else{
        h= jbdgauss(f,a,2.*s-b,eps,ap);
        b0=b-s;}
      c=cst/b0;
      bb=0;
