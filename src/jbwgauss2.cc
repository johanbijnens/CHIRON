...
dcomplex jbwgauss2(dcomplex (*f)(const dcomplex),const dcomplex a,
  ...
  /*
     ...
     jbwgauss2 is set equal to the approximate value of the integral of
...
{ const double x[13]={0.0
 ...
 dcomplex jbdgausst; 
 dcomplex aa,bb,c1,c2,s8,s16,u;
 double cconst;
 int i,ndiv;
 /*  start. */
 jbdgausst=dcomplex(0.);
  if(b == a) return jbdgausst;
  cconst=0.005/abs(b-a);
  bb=a;
  ndiv = 1;
  /*  computational loop. */
label1: 
...
label2:
  ...
  s16=c2*s16;
  if( abs(s16-s8)<= eps*(1.0+abs(s16)) ) goto label5;
  ndiv += 1;
  bb=c1;
  if( 1.0+abs(cconst*c2) != 1.0 ) goto label2;
  jbdgausst=dcomplex(0.0);
  std::cout << "jbwgauss2 too high accuracy required\n";
  return jbdgausst;
label5:
...
label2:
  ...
  for (i=1; i<=4;i++)
    { u=c2*x[i];
      s8=s8+w[i]*((*f)(c1+u,ap)+(*f)(c1-u,ap));
    }
  s8=c2*s8;
  s16=dcomplex(0.0);
  for (i=5;i<=12;i++)
    { u=c2*x[i];
      s16=s16+w[i]*((*f)(c1+u,ap)+(*f)(c1-u,ap));
    }
  s16=c2*s16;
  if( abs(s16-s8)<= eps*(1.0+abs(s16)) ) goto label5;
  ndiv += 1;
